...
/**
 * Servlet implementation class GuestModifyAction
 */
...
public class GuestModifyActionServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 
		 * 0.요청객체encoding설정
		 * 1.파라메타받기(guest_no,guest_name,guest_email,guest_homepage,guest_title,
		 * guest_content) Guest객체생성 2.GuestService객체생성 3.GuestService객체
		 * updateGuest(Guest객체) 메쏘드호출 4.guest_view.do로 redirection
		 */
		try {
			if (request.getMethod().equalsIgnoreCase("GET")) {
				...
				// forwardPath = String.format("redirect:guest_view.do?guest_no=%s",
				// guest_noStr); //printf의 포멧과 동일하다
